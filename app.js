...
const { loadSubscriptions } = require('./commands/watch');
const fs = require("fs");
...
bot.on('ready', () => {
    loadSubscriptions(bot); // initialize subscriptions in watch.js
});

// Token needed in config.json
bot.login(process.env.TOKEN_DISCORD).catch(console.error);